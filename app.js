...
io.sockets.on("connection", function(socket) {
  ...
  socket.on("changeCards", data => {
    ...
    game.packList.forEach(packName => {
      if (data.pack === packName) {
        // Toggle packs in the game
        game.activePacks[packName] = !game.activePacks[packName]
      }
    })
    // If all options are disabled, re-enable the base pack
    let allInactive = Object.values(game.activePacks).every(active => !active)

    if (allInactive) {
      game.activePacks.base = true
    }
    game.updateWordPool()
    gameUpdate(room)
  })
...
setInterval(() => {
  ...
  // AFK Logic
  for (let player in PLAYER_LIST) {
    PLAYER_LIST[player].afktimer-- // Count down players afk timer
